...
real substation's county. CATS node counts come from the same candidate-node
filter used by map_loads_to_nodes.py (Type='Substation', non-IMPORT).

Generalizable to any node system via --nodes/--id-col/--lat-col/--lon-col/
--filter (same conventions as map_loads_to_nodes.py).
...
Usage:
  python scripts/load_projection/plot_coverage_map.py
...
sys.path.insert(0, str(ROOT / "scripts/load_projection"))
from map_loads_to_nodes import SYNTHETIC_COUNTIES  # noqa: E402
