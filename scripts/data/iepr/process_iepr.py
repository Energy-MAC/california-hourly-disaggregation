...
_HOURLY_OUT_COLS = (
    ["forecast_vintage_year", "utility_ba", "scenario",
     "YEAR", "MONTH", "DAY", "HOUR", "hour", "datetime_pst"]
    + _HOURLY_LOAD_COLS
)
...
def process_hourly(out_dir: Path = OUT_DIR) -> Path:
    ...
    for f in files:
        ...
        # Construct hour-beginning PST datetime (IEPR HOUR is 1-24 hour-ending fixed PST = UTC-8)
        df["datetime_pst"] = (
            pd.to_datetime(dict(year=df["YEAR"].astype(int),
